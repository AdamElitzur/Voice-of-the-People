...
if (!process.env.CLERK_SECRET_KEY) {
    console.warn(
        "[WARN] Missing CLERK_SECRET_KEY. Clerk-protected routes will fail until this is set."
    );
}
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    console.warn(
        "[WARN] Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Supabase client will not be fully functional."
    );
}
if (!PINECONE_API_KEY) {
    console.warn(
        "[WARN] Missing PINECONE_API_KEY. Pinecone routes will fail until this is set."
    );
}
if (!OPENAI_API_KEY) {
    console.warn(
        "[WARN] Missing OPENAI_API_KEY. Embedding generation will fail until this is set."
    );
}
...
// Middleware
// Respect CORS_ORIGIN env (supports comma-separated list). Allows Authorization header for Clerk.
const corsOrigins =
    CORS_ORIGIN === "*"
        ? true
...
app.post("/api/query", async (req, res) => {
    try {
        if (!openai || !pineconeIndex) {
            return res
                .status(500)
                .json({ error: "Server missing OpenAI or Pinecone configuration" });
        }
        const { text } = req.body || {};
        let { topK } = req.body || {};
...
async function upsertResponseEmbedding({
    campaignId,
    responseId,
    text,
    extraMetadata = {},
    ...
    try {
        ...
        if (campaignIds.length) {
            ...
            for (const row of respRows || []) {
                if (!countsByCampaign[row.campaign_id])
                    countsByCampaign[row.campaign_id] = 0;
...
app.post("/api/campaigns/:id/ask", requireAuth(), async (req, res) => {
    try {
        ...
        const contexts = (result.matches || []).map(
            (m) => m.metadata?.text || m.metadata?.title || ""
        );
        const systemPrompt =
